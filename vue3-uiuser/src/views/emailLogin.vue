...
const loginRules = {
  email: [
    { required: true, trigger: "blur", message: "Please enter your email account" },
    { type: "email", trigger: "blur", message: "Please enter a valid email address" }
  ],
  code: [{ required: true, trigger: "blur", message: "Please enter the email verification code" }]
};
...
const loginResourceType = ref("video"); // image 或 video
...
function handleSendCode() {
  if (!loginForm.value.email) {
    ElMessage.error("Please enter your email account first");
    return;
  }
  // 验证邮箱格式
