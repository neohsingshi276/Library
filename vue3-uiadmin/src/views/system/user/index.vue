...
const upload = reactive({
  // Whether to show popup layer (user import)
  open: false,
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('Are you sure to delete user with ID "' + userIds + '"?').then(function () {
...
function handleStatusChange(row) {
  ...
  proxy.$modal.confirm('Are you sure to "' + text + '" user "' + row.userName + '"?').then(function () {
...
function handleResetPwd(row) {
  proxy.$prompt('Please enter new password for "' + row.userName + '"', "Tip", {
    ...
    resetUserPwd(row.userId, value).then(response => {
      proxy.$modal.msgSuccess("Updated successfully, new password is: " + value);
    });
  }).catch(() => {});
};
/** Selection change */
